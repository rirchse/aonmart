...
<div class="card card-custom">
    <div class="card-header">
        <div class="card-title">
            <h3 class="card-label">Edit Expense</h3>
        </div>
    ...
    <div class="card-body">
        <form class="kt-form kt-form--label-right" enctype="multipart/form-data" method="POST" action="{{ route('expenses.update', $expense->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="kt-portlet__body">
                        ...
                        <div class="col-lg-8">
                            <select name="store_id" id="store_id" class="custom-select  @error('store_id') is-invalid @enderror">
                                @foreach($stores as $store)
                                    <option value="{{ $store->id }}" {{ old('store_id', $expense->store_id) == $store->id ? 'selected' : null }}>{{ $store->name }}</option>
                                @endforeach
                            </select>
                            @error('store_id')
                    ...
                    <div class="col-lg-8">
                        <select name="expense_category_id" id="expense_category_id" class="custom-select  @error('expense_category_id') is-invalid @enderror">
                            ...
                            @foreach($expenseCategories as $expenseCategory)
                                <option value="{{ $expenseCategory->id }}" {{ old('expense_category_id', $expense->expense_category_id) == $expenseCategory->id ? 'selected' : null }}>{{ $expenseCategory->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-8">
                        <input name="date" id="date" value="{{ old('date', $expense->date->format('Y-m-d')) }}" placeholder="Ex: " type="date" class="form-control  @error('date') is-invalid @enderror">
                        @error('date')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-8">
                        <input name="amount" id="amount" value="{{ old('amount', $expense->amount) }}" placeholder="Ex: " type="text" class="form-control  @error('amount') is-invalid @enderror">
                        @error('amount')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-lg-8">
                        <textarea class="form-control @error('purpose') is-invalid @enderror" name="purpose" placeholder="Enter a expense purpose..." rows="4">{{ old('purpose', $expense->purpose) }}</textarea>
                        @error('purpose')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
...
@push('script')
    <script>
        $(document).ready(function () {
            ...
            const current_employee = '{{ old('expense_by', $expense->expense_by) }}';
