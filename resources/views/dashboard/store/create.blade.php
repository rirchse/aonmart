  ...
  <div class="card card-custom">
    <div class="card-header">
      <div class="card-title">
        <h3 class="card-label">New Store </h3>
      </div>
    </div>
      ...
      <form enctype="multipart/form-data" method="POST" action="{{ route('store.store') }}">
          ...
          <div class="col-lg-8 offset-lg-2">
            ...
            <div class="image-input image-input-empty image-input-outline" id="image" style="background-image: url({{ getImageUrl() }});background-size:cover;">
              ...
              <label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="tooltip" title="" data-original-title="Change avatar">
                ...
                <input type="file" value="{{ old('image') }}" name="image" onchange="readURLIcon(this)" accept=".png, .jpg, .jpeg"/>
        ...
        <div class="form-group row">
          <div class="col-lg-8 offset-lg-2">
            ...
            <input name="name" id="name" value="{{ old('name') }}" placeholder="Ex: Store Name" type="text" class="form-control  @error('name') is-invalid @enderror">
            @error('name')
          ...
          <div class="col-lg-8 offset-lg-2">
            ...
            <select name="categories[]" id='categories' class="form-control select2" multiple>
              @foreach ($categories as $category)
                <option value="{{ $category->id }}" {{ collect(old('categories'))->contains($category->id) ? 'selected' : null }}>{{ $category->name }}</option>
              @endforeach
            </select>
            @error('categories')
        ...
        <div class="form-group row">
          <div class="col-lg-8 offset-lg-2">
            ...
            <input name="shipping_fee" id="shipping_fee" value="{{ old('shipping_fee', 0) }}" placeholder="Ex: Store Name" type="number" class="form-control  @error('shipping_fee') is-invalid @enderror">
            @error('shipping_fee')
            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
            @enderror
          </div>
        ...
        <div class="form-group row">
          <div class="col-lg-8 offset-lg-2">
            ...
            <textarea name="address" id="address" rows="3" placeholder="Ex: Store Address" class="form-control  @error('address') is-invalid @enderror">{{ old('address') }}</textarea>
            @error('address')
            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
            @enderror
          </div>
        ...
        <div class="form-group row">
          <div class="col-lg-8 offset-lg-2">
            ...
            <input name="capital" id="capital" value="{{ old('capital', 0) }}" placeholder="Ex: Store Name" type="number" class="form-control  @error('capital') is-invalid @enderror">
            @error('capital')
            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
            @enderror
          </div>
        ...
        <div class="form-group row">
          <div class="col-lg-8 offset-lg-2">
            ...
            <textarea name="capital_note" id="capital_note" rows="3" placeholder="Ex: Store capital_note" class="form-control  @error('capital_note') is-invalid @enderror">{{ old('capital_note') }}</textarea>
            @error('capital_note')
            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
            @enderror
          </div>
        ...
        <div class="form-group row">
          <div class="col-lg-8 offset-lg-2">
            ...
            <select name="status" id="status" class="custom-select  @error('status') is-invalid @enderror">
              ...
              <option value="0" {{ old('status', true) == 0 ? 'selected' : '' }}>Inactive</option>
            </select>
            @error('status')
            <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
            @enderror
