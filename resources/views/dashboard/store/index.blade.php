  ...
  <div class="card card-custom">
    <div class="card-header">
      <div class="card-title">
        <h3 class="card-label">Stores </h3>
      </div>
      <div class="card-toolbar">
        @can('store.create')
          <a href="{{ route('store.create') }}" class="btn btn-primary font-weight-bolder">
      ...
      <table class="table" id="myTable" width="100%">
        ...
        @foreach ($stores as $store)
          <tr>
            ...
            <td>
              <img src="{{ getImageUrl($store->image) }}" alt="avatar" height="50px" width="50px"/>
            </td>
            <td>
              {{ $store->address }}
            </td>
            ...
            <td>
              <form method="post" action="{{ route('store.destroy', $store->id) }}">
                <a href="{{ route('store.edit', $store->id) }}" class="btn btn-sm  btn-clean btn-icon"><i class="la la-edit icon-lg"></i></a>
                @csrf
                @method('DELETE')
