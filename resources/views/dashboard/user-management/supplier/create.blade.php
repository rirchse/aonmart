  ...
  <div class="card card-custom">
    <div class="card-header">
      <div class="card-title">
        <h3 class="card-label">New Supplier</h3>
      </div>
    </div>
    <div class="card-body">
            ...
            <div class="col-md-8">
              ...
              <select class="form-control select2-withTag" name="company_id" id='company_id'>
                ...
                @foreach ($companies as $company)
                  <option value="{{ $company->name }}" {{ collect(old('company_id'))->contains($company->name) ? 'selected' : null }}>{{ $company->name }}</option>
                @endforeach
              </select>
              @error('company_id')
            ...
            <div class="offset-lg-2 col-lg-8">
              ...
              <select class="form-control select2" multiple id="product_id" name="product_id[]">
                @foreach ($products as $product)
                  <option value="{{ $product->id }}" {{ collect(old('product_id'))->contains($product->id) ? 'selected' : null }}>{{ $product->name }}</option>
                @endforeach
              </select>
              @error('product_id')
              <span class="text-danger"><strong>{{ $message }}</strong></span>
